...
// pub fn calculate_member_stress(truss: Truss) {
//     let size = 2 * truss.pos.len();
//     let mut reactions = 0;
//     let mut matrix = Mat::<f32>::zeros(size, size);
//     //maybe preallocate by size eventually bc we know connectivity_mat size
//     let ssize = size / 2;
//     let mut connectivity_mat = Mat::<bool>::with_capacity(ssize, ssize);
//     let length = size / 2;
//     let mut i = 0;
//     let mut j = 0;
//     let mut k = 0;
...
//             } else {
//                 connectivity_mat[(i, j)] = truss.mat[k];
//                 k += 1;
//             }
//             j += 1;
//         }
